...
#include <fstream>
    ...
    /*CODICO SIMULANDO O PROTOCOLO Rtp 2.0 E Rtp 2.1:
...
int delay = 0, erro = 0, flag_seqnum = 0,flag_acknum=0;
...
void communication_network() {
        ...
        cout <<endl<<"Numero randomico gerado: ";
        ...
        if (altera_pacote == 1) {
            ...
            delay = 1;
            cout << "Ocorreu um atrado na estrega do pacote, aguarde" << endl;
            Sleep(2000);// atraso
        }
...
void transport_layer_input_B() {
    ...
    estados_op = s1;
    while (estados_op != s3 && pacote_Alice.acknum == 0) {
        // cout<<"trasnp layer in b\n";
        switch (estados_op) {
    ...
    if (estados_op == s3) {
        cout << "Problema no envio, sera refeito"<<endl;
        system("pause");
    }
}
...
void fun_app_receive() {
    ...
    cout<<"A mensagem recebida foi essa: ";
    cout<<mensagem<<endl;
}
...
int main() {
  ...
  for (int i = 0; i < 5; i++)
  {
        ...
        app_layer_A();
        cout <<endl<< "DESEJA MANDAR OUTRA MENSAGEM? DIGITE 1 PARA SIM, 0 PARA NAO: ";
        cin >> opcao;
        if (opcao == 1) {
            mandar_mensg = true;
            num_mensg++;
